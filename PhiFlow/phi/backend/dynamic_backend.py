...
class DynamicBackend(Backend):
    ...
    def split(self, tensor, num_or_size, axis=0):
        return self.choose_backend([tensor]).split(tensor,num_or_size,axis)

    def roll(self, tensor, shift, axis):
        return self.choose_backend([tensor]).roll(tensor,shift,axis)
