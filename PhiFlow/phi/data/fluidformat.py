...
class Scene(object):
    ...
    def copy_calling_script(self, full_trace=False, include_context_information=True):
        ...
        for script_path in script_paths:
            _copy_file(script_path, join(self.subpath('diffpiso', create=True), os.path.basename(script_path)))
        if include_context_information:
            with open(join(self.subpath('diffpiso', create=True), 'context.json'), 'w') as context_file:
                json.dump({
    ...
    def copy_src(self, path, only_external=True):
        if not only_external or not _is_phi_file(path):
            _copy_file(path, join(self.subpath('diffpiso', create=True), os.path.basename(path)))
